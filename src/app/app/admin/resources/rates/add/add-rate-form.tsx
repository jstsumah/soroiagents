...
export function AddRateForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      toast({ title: "Starting upload...", description: "Please wait while files are being uploaded." });
      ...
      toast({ title: "Files uploaded.", description: "Finalizing rate card creation..." });
      
      const rateData: Omit<Rate, 'id'> = {
        title: values.title,
      ...
      toast({
        ...
        description: "The new rate card has been successfully created.",
      });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                ...
                render={({ field: { onChange, onBlur, name, ref } }) => (
                    ...
                    <FormDescription>The main image displayed on the card. Max size: {MAX_FILE_SIZE_MB}MB.</FormDescription>
                    <FormMessage />
                    </FormItem>
                )}
                />
                ...
                render={({ field: { onChange, onBlur, name, ref } }) => (
                    <FormItem>
                    <FormLabel>PDF Rate File (Optional)</FormLabel>
                    <FormControl>
                        <Input type="file" accept=".pdf" onChange={(e) => onChange(e.target.files)} onBlur={onBlur} name={name} ref={ref} disabled={isUploading}/>
                    </FormControl>
                    <FormDescription>The downloadable rate document. Max size: {MAX_FILE_SIZE_MB}MB.</FormDescription>
                    <FormMessage />
                    </FormItem>
                )}
                />
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button type="submit" disabled={isUploading}>
                ...
                {isUploading ? "Uploading..." : "Add Rate Card"}
              </Button>
            </div>
          </form>
        </Form>
