...
export function HowToGetThereForm({ locationData }: { locationData?: HowToGetThereLocation }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: locationData || {
      name: "",
      mapUrl: "https://picsum.photos/1200/600",
      tier_access: [],
      flights: [],
      trains: [],
      roads: [],
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
        if (isEditMode && locationData) {
            await updateLocation(locationData.id, values);
            toast({
                title: "Location Updated!",
                description: `The location "${values.name}" has been successfully saved.`,
            });
        } else {
            await addLocation(values as Omit<HowToGetThereLocation, 'id'>);
            toast({
            ...
            <CardContent className="space-y-6">
                ...
                <FormField control={form.control} name="mapUrl" render={({ field }) => (
                    <FormItem>
                        ...
                        <FormControl><Input placeholder="https://your-map-image-url.com" {...field} /></FormControl>
                        <FormMessage />
                    </FormItem>
                )} />
